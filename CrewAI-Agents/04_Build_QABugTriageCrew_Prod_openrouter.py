...
# OpenRouter routes to DeepSeek (openrouter/deepseek/deepseek-v4-flash) and
# many other models with one API key. There is no per-request 8000-token cap
# the way Groq's free tier has, so every agent gets a comfortable output
# budget for its report.
def make_llm(max_tokens):
    return LLM(
...
# Budgets: the reports grow as context is chained, so the last agent gets the
# most room. These are output-token ceilings; OpenRouter handles the rest.
triage_llm = make_llm(4000)  # smallest prompt, runs first
rca_llm = make_llm(5000)     # prompt also carries task 1's output
sdet_llm = make_llm(6000)    # prompt also carries task 1 + task 2 output

print(f"[llm] provider: {os.getenv('OPENROUTER_MODEL', 'openrouter/deepseek/deepseek-v4-flash')}")
...
if __name__ == "__main__":
    ...
    if usage and usage.total_tokens:
        ...
        print("✅ Done. (Token usage was not reported by the provider.)")
    print("=" * 70)
